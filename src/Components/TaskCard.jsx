...
export default function TaskCard({_id, title,description,tag}) {
  ...
  return (
    <div className='mx-auto py-4  border border-gray-400 w-80 md:w-160 lg:w-240 xl:w-340 rounded-md'>
      
      <div className='flex flex-col w-full '>
        
        <div className='flex flex-col md:flex-row justify-between text-right md:items-center mb-4 w-full px-8'>
          <p className={`font-semibold px-3 py-1 rounded-md inline-block
          ...
          <div className='flex flex-col md:flex-row gap-4'>
           <Link to={`/edit/${_id}`}>
            ...
            <button onClick={handleOnClick} className='text-purple-400 border border-purple-400 bg-white py-2 px-4 rounded-md flex gap-2 cursor-pointer'>
        ...
        <div className='px-8 flex flex-col gap-4'>
          <p className='text-4xl font-semibold wrap-break-word whitespace-normal'>{title}</p>
          <p className='text-gray-600 text-xl wrap-break-word whitespace-normal'>{description}</p>
        </div>
      </div>
    </div> 
  )
